...
1. **【触发与缓冲区收集】** 当 `VSYNC-sf` 信号到达时，SurfaceFlinger 被唤醒。它会遍历所有可见图层，从每个图层对应的 `BufferQueue` 中获取（acquire）最新的、已准备就绪的缓冲区。如果没有新缓冲区提交，则继续使用之前获取的缓冲区，以确保系统始终有内容可显示。同时，也会发送 `VSYNC-app` 信号，唤起上段介绍的Choreographer
