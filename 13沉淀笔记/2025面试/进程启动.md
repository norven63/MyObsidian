...
5.  **建立系统连接（Binder注册）**
    ...
    *   至此，AMS获得了指挥该App进程的能力，并且通过调用`ApplicationThread.bindApplication()`回到App进程，最终一步步调用到App进程的 **`ActivityThread.installProvider()`** → `Application.onCreate()`
...
6.  **完成应用启动（生命周期回调）**
    *   AMS通过拿到的“遥控器”（`ApplicationThread`），向新进程发送系列Binder指令：
        *   `BIND_APPLICATION`: 触发调用`ApplicationThread.bindApplication()`回到App进程，最终一步步调用到App进程的 **`ActivityThread.installProvider()`** → 创建`Application`并调用其`onCreate()`方法
        *   后续指令（如`LAUNCH_ACTIVITY`）：启动最初请求的组件（如Activity）


#### Application创建过程

![[Pasted image 20251019165828.png]]
