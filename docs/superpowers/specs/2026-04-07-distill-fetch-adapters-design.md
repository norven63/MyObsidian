...
首版允许增加一个**best-effort 试点站点**：

- `zhihu`

但 `zhihu` 不属于首版放行的阻断条件，且默认状态为 **disabled**。只有在**公开页面、无登录、无人工注入私有会话**的前提下，能够稳定达到高质量抓取门槛时，才允许作为正式站点规则启用；否则保持禁用状态，并始终按未知站点路径 `generic -> agent_fallback` 处理。

不在首版内置更多站点规则。新增站点需满足：
...
V1 只支持**公开可访问、无需登录的页面抓取**。  
以下方式不纳入首版正式方案：

- 使用用户私有 cookie / token / 登录态辅助抓取
- 依赖人工打开浏览器后导出的临时会话
- 需要账号权限、付费权限或验证码交互才能稳定获取正文

如果某站点（包括 `zhihu`）只有在上述前提下才能拿到高质量正文，则该站点规则在 V1 中不得正式放行。
...
知乎在 V1 中只作为 **best-effort public-page pilot**：

- 仅尝试公开可访问的问题回答页 / 文章页
- 不依赖登录态、cookie 注入或人工浏览器会话
- 若页面直接返回登录墙、风控页、截断页或非正文页，则判硬失败
- 若能稳定提取回答正文、作者、发布日期，并达到质量门要求，才允许进入正式候选仲裁
- 若质量长期不稳定或只能靠登录态获取完整正文，则不启用 `zhihu` 专用规则
- 未启用前，`zhihu` 不参与 known-site routing，也不影响 V1 的阻断门禁

也就是说，`zhihu` 的目标是“能加分就加”，而不是“做不出来就阻断 V1”。

启用检查清单至少包括：

1. 公开样本集上连续通过质量门
2. 不依赖登录态或私有凭据
3. 回滚开关已定义，禁用后自动退回 `generic -> agent_fallback`
...
4. metadata 冲突样本
5. 截断正文样本
6. 知乎公开页样本（仅在 `zhihu` 规则启用时纳入阻断门禁）
