...
### 2.4 关键条件 / 例外 / 易错分叉
- **先分清“系统主线慢”还是“业务初始化把关键路径拉长”**：很多首帧问题并不是 framework 某层天然慢，而是 app 自己把 provider、`Application`、首屏准备排成了长串行链。
- **主线程看起来空闲，不代表输入和渲染链健康**：事件可能还堵在输入分发，帧也可能晚在 `RenderThread` / `SurfaceFlinger`，而不是 Java 主线程。
- **点击没落到目标窗口，优先怀疑焦点 / 可触摸区域 / 输入窗口列表，而不是先改业务回调**：窗口与输入路由层的错位，表面上很像“onClick 没执行”。
- **别把每个症状都直接下钻到最底层源码**：只有先确定落在哪条责任链，再决定是否继续追 Binder 驱动、内核调度或 GPU / HWC 实现，排障成本才可控。
...
- **关键条件 / 例外 / 易错分叉**：如果 process start、attach、bindApplication 这条线都正常，首帧却仍明显偏晚，就别继续把锅留在 AMS / Zygote，而该转去看 app 自己的初始化和首屏准备；反过来，如果实例都还没真正拉起，再盯页面逻辑只会跑偏。
- **诊断 / 应用抓手**：
...
- **关键条件 / 例外 / 易错分叉**：看到主线程“像在等”时，先分清它是在等消息到期、被同步屏障改了节奏，还是根本在等 Binder / I/O 回来；这些表象都可能长得像“主线程没干活却卡住了”，但归因层次完全不同。
- **诊断 / 应用抓手**：
...
- **关键条件 / 例外 / 易错分叉**：窗口“看得见却点不到”时，先查焦点、可触摸区域和窗口标志；如果事件其实已经进了目标窗口，再回到 View 事件处理和渲染反馈层。别把系统路由问题和业务回调问题混成一类。
- **诊断 / 应用抓手**：
...
- **关键条件 / 例外 / 易错分叉**：主线程空闲并不等于这一帧安全，真正的瓶颈可能已经转移到 RenderThread、GPU 或 SurfaceFlinger；但如果输入事件本身还没到窗口，就不该先怪渲染链，得先回到输入分发层。
- **诊断 / 应用抓手**：
