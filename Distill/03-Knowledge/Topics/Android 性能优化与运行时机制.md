...
### 2.4 关键条件 / 例外 / 易错分叉
- **先确认问题是“时间分布异常”还是“语义决策错误”**：启动慢和 task / back stack 复用错常一起出现，但排查入口完全不同，不能混成一个问题去修。
- **主线程不忙，不代表画面一定没问题**：输入、`RenderThread`、`SurfaceFlinger`、GPU deadline 任一层晚到，都能造成“线程栈不重但用户仍觉得卡”。
- **内存问题先证明“谁让对象活着”**：只看 RSS / PSS 总量或 GC 次数，容易把缓存增长、Bitmap 峰值和真实泄漏混成一类。
- **症状跨多层且复现不稳时，先统一时间线再局部下钻**：没有同一份 Perfetto / marker 视角，分散日志很容易把先后因果看反。
...
- **关键条件 / 例外 / 易错分叉**：如果返回栈语义已经错了，先修导航决策而不是把它误报成性能问题；如果实例复用正确但首帧晚，再沿 process start、binder wait、first frame 去分时间，不要一边改 launchMode 一边猜耗时。
- **诊断 / 应用抓手**：
...
- **关键条件 / 例外 / 易错分叉**：线程 blocked 只是表象，关键是确认它在等 system_server、别的 binder 端点，还是本进程锁；如果还没证明“谁在等谁”，就不该先把问题拍成“App 主线程慢”。
- **诊断 / 应用抓手**：
...
- **关键条件 / 例外 / 易错分叉**：如果事件还没到目标窗口，就先查输入链；如果事件已经到达、主线程也处理完了，但画面仍晚到，才把重心放到 RenderThread / SurfaceFlinger / GPU。别把“手到了，画面没到”自动等同于主线程卡。
- **诊断 / 应用抓手**：
...
- **关键条件 / 例外 / 易错分叉**：内存高不等于泄漏，峰值分配和真实 retained set 要分开看；但如果页面退出后对象仍持续被支配，再拿“只是缓存”当解释，通常就是误判。
- **诊断 / 应用抓手**：
