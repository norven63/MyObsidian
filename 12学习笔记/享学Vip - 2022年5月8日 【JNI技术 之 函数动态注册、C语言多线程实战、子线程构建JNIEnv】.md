...
> - 线程的终止，可以是隐式的退出（执行完函数，或者执行到 `return` 语句），也可以显式的调用 `pthread_exit()` 函数来退出。
> - pthread_exit函数唯一的参数value_ptr是函数的返回代码，只要 `pthread_join()` 中的第二个参数value_ptr不是NULL，这个值将被传递给value_ptr。


2. **pthread_join** 函数：`int pthread_join(pthread_t thread, void **value_ptr);`

>函数 `pthread_join()` 的作用是，等待一个线程终止。
