...
### 二、拷贝构造函数
- **浅拷贝 (Shallow Copy)** ：只复制某个对象的指针，而不复制对象本身，新、旧对象还是共享同一块内存。例如 `this->name = stu.name;`、`Student *st; Student *st2 = st;`
- **深拷贝 (Deep Copy)** ：在拷贝的过程中会在堆中开辟一个新的内存区域另外创造一个一模一样的对象，新对象跟原对象不共享内存，修改新对象不会改到原对象.
- **隐式的调用拷贝构造函数的场景**：
...
void objectMain() {
	...
	Student stu3;
	stu3 = stu1; // 2. 【这样的赋值写法“不会”调用自定义拷贝构造函数，仅仅只是执行了成员的“浅拷贝”逻辑】
