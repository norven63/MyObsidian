...
- **隐式的调用拷贝构造函数的场景**：
	...
	2. 调用函数，将对象当作形参传入时
- 拷贝构造函数是默认就存在的，但我们可以通过覆写它（ 示例：`Student(const Student& student){ ... }` ），来实现自定义对象拷贝的逻辑。
- 隐式的调用拷贝构造函数时，如果是默认逻辑（没有覆写），则把旧对象的成员值**浅拷贝**给新对象（注意字符串陷阱）；如果覆写过拷贝构造函数，则按照自定义覆写的逻辑进行赋值。
- 默认的拷贝构造函数逻辑是浅拷贝：
	1. 先寻址到 **旧对象** 成员的值
	2. 再赋值给 **新对象** 的成员（浅拷贝）
