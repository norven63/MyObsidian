...
- **构造函数：**
	- 无参构造函数的触发调用场景。`Student stu;` 、`Student stu = new Student();`
	- 有参构造函数的触发调用场景。`Student stu("雄霸");`、`Student stu = new Student("雄霸");`
	- 构造函数嵌套调用。`Student::Student(char* pName) :Student(pName, 87){ ... }`
	- 构造函数在函数签名上，直接编写给成员赋值的逻辑。`Student::Student(int pAge) :age(pAge){ ... }`
- **析构函数：**
...
### 二、拷贝构造函数
- 拷贝构造函数是默认就存在的，我们可以通过覆写它（ 示例：`Student(const Student& student){ ... }` ），来实现自定义拷贝逻辑的目的
- 当执行 `Class名 新对象 = 旧对象;` 逻辑时，就会隐式的调用拷贝构造函数。如果是默认逻辑，则把旧对象的成员值拷贝给stu2；如果覆写过拷贝构造函数，则按照自定义覆写的逻辑进行赋值。
- 默认的拷贝构造函数逻辑是浅拷贝：
	1. 先寻址到 **旧对象** 成员的值
	2. 再赋值给 **新对象** 的成员（浅拷贝）
