...
### 三、关键字：this
1. 当声明对象时，**构造函数**会创建一个`this`的指针，指向当前对象的地址。并且**用`const`修饰成"指针常量"**，使其指向地址不可修改，但是成员变量允许修改。
2. 当给成员函数的末尾修饰`const`时，等价于给该函数传入的`this`指针，修饰成**常量指针常量**。例如 `void foo() const { ... }`
3. 当对象的**成员函数**都在**代码区**运行，当被调用时，会传入对象的`this`指针到代码区。同理，构造函数、栈区空间，也会传入。
