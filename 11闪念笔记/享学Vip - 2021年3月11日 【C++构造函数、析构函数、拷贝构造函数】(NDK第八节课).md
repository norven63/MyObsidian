...
### 二、拷贝构造函数
- 拷贝构造函数是默认就存在的，我们可以通过覆写它，来实现自定义拷贝逻辑
...
void objectMain() {
	...
	Student stu2 = stu1;  // 1. 这样的赋值写法会调用自定义拷贝构造函数
	cout << "main函数中 student 的内存地址: " << &stu1 << endl; // 地址的打印与自定义拷贝函数中是一样的

	Student stu3;
	stu3 = stu1; // 2. 这样的赋值写法“不会”调用自定义拷贝构造函数，仅仅只是进行了成员的值拷贝逻辑
